...
class ChartManager {
  ...
  // 初始化图表
  init() {
    this.udf_datafeed = new Datafeeds.UDFCompatibleDatafeed(window.API_BASE_URL + "/tv", 30000);
    this.widget = window.tvWidget = new TradingView.widget({
      ...
      charts_storage_url: window.API_BASE_URL + "/tv",
  ...
  setupEventListeners() {
    ...
    this.widget.headerReady().then(function () {
      ...
      buttonDeleteMark.addEventListener("click", function () {
        ...
        $.post({
          type: "POST",
          url: window.API_BASE_URL + "/tv/del_marks",
          dataType: "json",
