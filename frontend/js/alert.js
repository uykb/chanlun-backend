  ...
  return {
    init: function () {
      layui.use(["table", "form"], function () {
        ...
        // 获取提醒任务列表并填充到select中
        $.get(window.API_BASE_URL + "/alert_list/" + Utils.get_market(), function (res) {
    ...
    get_alert_records: function () {
      layui.use(["table", "form"], function () {
        ...
        table.render({
          ...
          url:
            window.API_BASE_URL + "/alert_records/" +
            Utils.get_market() +
            "?task_name=" +
    ...
    refresh_alerts_table: function () {
      layui.use(["table", "dropdown", "util"], function () {
        ...
        table.render({
          elem: "#table_alerts",
          defaultContextmenu: false,
           url: window.API_BASE_URL + "/alert_list/" + Utils.get_market(),
        ...
        table.on("row(table_alerts)", function (obj) {
          ...
          layer.open({
            ...
            title: "修改警报提醒",
            area: ["1000px", "90vh"],
            content: window.API_BASE_URL + "/alert_edit/" + Utils.get_market() + "/" + data.id,
          ...
          dropdown.render({
            ...
            click: function (menuData, othis) {
              if (menuData["id"] === "del") {
                $.ajax({
                  type: "GET",
                   url: window.API_BASE_URL + "/alert_del/" + data.id,
